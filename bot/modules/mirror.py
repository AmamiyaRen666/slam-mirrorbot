    ...
    def onUploadComplete(self, link: str, size):
        with download_dict_lock:
            ...
            if uname is not None:
                msg += f'\n\n<b>➩ 👤 𝗕𝘆 :</b> {uname} 𝑰𝑫 : <code>{self.message.from_user.id}</code> ✨\n\n<b>➩ 🗳 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 @AboutReyshaNim</b>\n'
            try:
...
def _mirror(bot, update, isTar=False, extract=False):
        ...
        if not bot_utils.is_url(link) and not bot_utils.is_magnet(link) or len(link) == 0:
            if file is not None:
                if file.mime_type != "application/x-bittorrent":
                    ...
                    sendMessage(f"<b>📝 Your Telegram File Has Been Added To Bot Queue...\n💡 Check Progress By 💢</b> /{BotCommands.StatusCommand} 💢", bot, update)
                    if len(Interval) == 0:
    ...
    if not bot_utils.is_url(link) and not bot_utils.is_magnet(link):
        sendMessage(f'<b>𝐎𝐩𝐩𝐬 🤦 \n\n📢 𝐈𝐟 𝐲𝐨𝐮 𝐃𝐨𝐧❜𝐭 𝐤𝐧𝐨𝐰 𝐡𝐨𝐰 𝐭𝐨 𝐮𝐬𝐞 𝐂𝐡𝐞𝐜𝐤 𝐎𝐭𝐡𝐞𝐫𝐬 𝐌𝐞𝐬𝐬𝐚𝐠𝐞 𝐨𝐫 𝐌𝐞𝐧𝐭𝐢𝐨𝐧 𝐀𝐃𝐌𝐈𝐍😐</b>\n\n<b>🏷️𝗘𝘅𝗮𝗺𝗽𝗹𝗲:</b>\n /{BotCommands.MirrorCommand} <b>Your Torrent Magnet or Direct URL Link</b>', bot, update)
        return
