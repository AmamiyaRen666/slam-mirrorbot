...
class MirrorStatus:
    STATUS_UPLOADING = "Uploading To Drives ...📤"
    STATUS_DOWNLOADING = "Downloading Your Files ...📥"
...
def get_readable_message():
    with download_dict_lock:
        ...
        for download in list(download_dict.values()):
            msg += f"「 🗂️ 𝗬𝗼𝘂𝗿 𝗙𝗶𝗹𝗲𝘀 𝗜𝗻 𝗣𝗿𝗼𝗰𝗲𝘀𝘀 𝗣𝗹𝗲𝗮𝘀𝗲 𝗪𝗮𝗶𝘁... 」"
            ...
            if download.status() != MirrorStatus.STATUS_ARCHIVING and download.status() != MirrorStatus.STATUS_EXTRACTING:
                ...
                try:
                    msg += f"\n<b>🔍 Tracker :- 🛩️ Seeds : {download.aria_download().num_seeders}</b>" \
                           f" | <b>✈️ Peers : {download.aria_download().connections}</b>"
                except:
                    pass
            if download.status() == MirrorStatus.STATUS_DOWNLOADING:
